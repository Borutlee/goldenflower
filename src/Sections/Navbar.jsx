        ...
        <div className="flex-shrink-0">
          <a href="/" className="flex items-center group gap-1 sm:gap-2 no-underline">
            ...
            <h1 className="text-lg sm:text-2xl font-black tracking-tighter">
              <span className="text-[#D4AF37] group-hover:text-[#B8860B] transition-all duration-500">
                GOLDEN
              </span>
              <span className="text-gray-800 dark:text-gray-300 inline transition-colors duration-300">
                {" "}FLOWER
              </span>
            </h1>
          </a>
        ...
        <nav className="hidden md:flex flex-grow items-center justify-center font-semibold">
          <ul className="flex list-none gap-2">
            {navLinks.map((item) => (
              <li
                key={item.label}
                className="group cursor-pointer px-5 py-1.5 border-2 border-transparent rounded-[30px] 
                hover:border-[#D4AF37] transition-all duration-300 flex items-center justify-center"
              >
                <Link
                  to={item.to}
        ...
        <div className="flex flex-col p-6 h-full text-gray-800 dark:text-white transition-colors duration-300">
          ...
          <nav className="flex flex-col gap-4 font-semibold text-lg">
            {navLinks.map((item) => (
              <Link
                ...
                className="py-2 border-b border-gray-50 dark:border-gray-700 
          ...
          <div className="mt-auto pt-10 flex flex-col gap-4">
            ...
            >
              ...
              {isDark ? (
                <MdOutlineLightMode className="text-yellow-500" size={22} />
              ) : (
                <MdDarkMode size={22} />
              )}
            </button>
